...
object Pref : KotprefModel()
{
    ...
    val isExternalOrSafStorage : Boolean get() {
        ...
        {
            return Uri.parse(rootPath.value).scheme == "content";
        }
        else
        {
            return Uri.parse(rootPath.value).path?.contains(Environment.getExternalStorageDirectory().absolutePath) ?: false
        }
    }
